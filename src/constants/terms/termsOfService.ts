export const TERMS_OF_SERVICE_TITLE = '이용약관';

export const TERMS_OF_SERVICE_CONTENT = `제1조 (목적)
이 약관은 이용자가 빼곡(이하 "서비스")에서 제공하는 모든 서비스의 이용 조건 및 절차에 관한 기본적인 사항을 규정함을 목적으로 합니다.

제2조 (용어의 정의)
이 약관에서 사용하는 용어의 정의는 다음과 같습니다.
"서비스"라 함은 구현되는 PC, 모바일 기기를 통하여 "이용자"가 이용할 수 있는 보장분석서비스 등 회사가 제공하는 제반 서비스를 의미합니다.
"이용자"란 "빼곡"에 접속하여 이 약관에 따라 "빼곡"이 제공하는 서비스를 받는 회원 및 비회원을 말합니다.
"회원"이란 "빼곡"에 개인정보를 제공하여 회원등록을 한 자로서, "빼곡"이 제공하는 서비스를 이용하는 자를 말합니다.
...
제3조 (약관의 효력 및 변경)
① 본 약관은 "빼곡" 내 또는 그 연결화면에 게시하거나 이용자에게 공지함으로써 효력이 발생합니다.
② 회사는 불가피한 여건이나 사정이 있을 경우 「약관의 규제에 관한 법률」, 「정보통신망이용촉진 및 정보보호 등에 관한 법률」 등 관련 법령에 위배하지 않는 범위에서 이 약관을 개정할 수 있습니다.
③ 회사가 약관을 개정할 경우에는 적용일자 및 개정내용, 개정사유 등을 명시하여 최소한 그 적용일 7일 이전부터 "빼곡" 내 또는 그 연결화면에 게시하여 이용자에게 공지합니다.
...
제4조 (약관 외 준칙)
이 약관에서 정하지 아니한 사항과 이 약관의 해석에 관하여는 「약관의 규제에 관한 법률」, 「정보통신망이용촉진 및 정보보호 등에 관한 법률」 등 관련 법령 또는 상관례에 따릅니다.

제5조 (회원의 의무)
① 회원은 회사에서 제공하는 서비스의 이용과 관련하여 다음 각 호에 해당하는 행위를 해서는 안 됩니다.
② 회원의 ID와 비밀번호에 관한 관리책임은 회원에게 있으며, 이를 제3자가 이용하도록 하여서는 안 됩니다.
...
6. 타인을 기망하여 이득을 취하는 행위, 회사의 서비스 이용과 관련하여 타인에게 피해를 입히는 행위
...
② 회사는 최근의 서비스 이용일부터 연속하여 3년 동안 회사의 서비스를 이용하지 않은 회원(이하 "휴면계정")의 개인정보를 보호하기 위해 이용계약을 정지 또는 해지하고 회원의 개인정보를 분리보관 또는 파기 등의 조치를 취할 수 있습니다. 이 경우 조치일 30일 전까지 계약 정지 또는 해지, 개인정보 분리보관 또는 파기 등의 조치가 취해진다는 사실 및 파기될 개인정보 등을 회원에게 통지합니다.
...
③ 회사는 계속적이고 안정적인 서비스 제공을 위하여 서비스 개선 중 설비에 장애가 생기거나 데이터 등이 멸실, 훼손된 때에는 천재지변, 비상사태, 현재의 기술로 해결이 불가능한 장애나 결함 등 부득이한 사유가 없는 한 지체 없이 이를 수리 또는 복구하도록 최선의 노력을 다합니다.

제8조 (서비스의 이용)
① "빼곡"은 다음과 같은 서비스를 회원에게 제공합니다.
1. 도서 검색 결과 제공
2. 독서기록 저장
3. 독서기록 푸시(PUSH) 알림

제9조 (개인정보의 보호 및 사용)
① 회사는 이용자의 정보 수집 시 원활한 서비스 제공을 위해 필요한 최소한의 정보를 수집합니다.
...
⑤ 기타 개인정보 관련 사항은 "빼곡"에 별도로 게시하는 개인정보처리방침에 의거합니다.

제10조 (회사의 면책)
① 회사는 서비스 제공이 일시적으로 중단됨으로 인하여 이용자 또는 제3자가 입은 손해에 대해서는 관련 법에 특별한 규정이 없는 한 별도의 보상을 하지 않습니다.
...
⑥ 회사는 회원이 "빼곡" 아이디 및 비밀번호, 계정정보 등을 관리하지 않아 발생하는 손해에 대해 책임을 지지 않습니다.
⑦ 회원이 모바일 기기의 변경, 모바일 기기 번호 변경, 운영체제(OS) 버전 변경, 해외 로밍, 통신사 변경 등으로 인해 콘텐츠 전부나 일부 기능을 이용할 수 없는 경우 회사는 이에 대해 책임을 지지 않습니다.
⑧ 회원이 회사가 제공하는 콘텐츠나 계정정보를 삭제한 경우 회사는 이에 대해 책임을 지지 않습니다.

제11조 (저작권 등의 귀속)
① 회사가 제작한 서비스 내 콘텐츠에 대한 저작권과 기타 지적재산권은 회사에 귀속합니다.
② 회원은 회사가 제공하는 서비스를 이용하여 얻은 정보 중에서 회사 또는 제공업체에 지적재산권이 귀속된 정보를 회사 또는 제공업체의 사전 동의 없이 복제, 전송 등의 방법(편집, 공표, 공연, 배포, 방송, 2차적 저작물 작성 등을 포함)에 의하여 영리목적으로 이용하거나 타인에게 이용하게 하여서는 안 됩니다.
...
② 회사는 회원으로부터 제기되는 의견이나 불만이 정당하다고 객관적으로 인정될 경우에는 합리적인 기간 내에 이를 신속하게 처리합니다. 다만, 처리에 장기간이 소요되는 경우에는 회원에게 장기간이 소요되는 사유와 처리일정을 서비스 내 공지하거나 통지합니다.
