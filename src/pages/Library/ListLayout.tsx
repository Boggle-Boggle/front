  ...
  return (
    <section className="p-4">
      {allBooks.map(({ readingRecordId, imageUrl, title, rating, recentReadDate, readingCount }) => {
        ...
        const endDate = recentReadDate?.endReadDate ? formatDateTimeToDate(recentReadDate.endReadDate) : '읽는중';
            ...
            <button className="h-full w-full" type="button">
              ...
              <div className="ml-32 flex h-full flex-col items-start py-[0.6rem]">
                <img
                  ...
                  className="w-[5.5rem]"
                />
                <p className="flex grow items-center justify-center pr-2 text-start font-semibold leading-6">{title}</p>

                <p className="text-[0.8125rem] opacity-50">
                  {!recentReadDate ? '아직 읽기 전이에요' : `${startDate} ~ ${endDate}`}
                </p>
              </div>
            </button>
            {readingCount > 0 && (
